...
from config import (BLOCK_FILES, BLOCK_LABELS, WEIGHT_LEVELS, DEFAULT_LEVELS,
                    LEVEL_OPTIONS, EXPLORE_TOP_N_TAGS, EXPLORE_RESULTS)
...
from controls import (render_presets, render_auto_tune_buttons,
                      render_controls, render_content_filters)
...
for name in BLOCK_FILES:
    st.session_state.setdefault(f"lvl_{name}", DEFAULT_LEVELS[name])
...
# ── Derive weights from canonical state ──
levels  = {name: st.session_state[f"lvl_{name}"] for name in BLOCK_FILES}
weights = {name: WEIGHT_LEVELS[levels[name]] for name in BLOCK_FILES}
...
with tab_similar:
    ...
    if album_id and not all(w == 0.0 for w in weights.values()):
        ...
        active = "  ·  ".join(f"{BLOCK_LABELS[k]} {levels[k]}"
