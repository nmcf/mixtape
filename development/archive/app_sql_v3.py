...
def highlight_unique(df, shared_ids):
    return [
        'background-color: #fff3cd; color: #1a1714; font-weight: 500;'
        if aid not in shared_ids else ''
        for aid in df['album_id']
    ]

def render_results(recs, label, shared_ids):
    st.markdown(f'**{label}**')
    if recs is None or recs.empty:
        ...
        st.dataframe(
            recs[['Album', 'Artist']].style.apply(
                lambda _: highlight_unique(recs, shared_ids),
    ...
    if matches.empty:
        ...
        if not album_options:
            ...
            if selected_album_name:
                ...
                if all(r is None or r.empty for r in [recs_v1, recs_v2, recs_v3]):
                    st.info('No recommendations available for this album.')
                else:
                    st.subheader('You might also like')

                    s1, s2, s3 = ids_set(recs_v1), ids_set(recs_v2), ids_set(recs_v3)
                    shared_ids = s1 & s2 & s3
                    ...
                    st.markdown(
                        ...
                        "<span style='font-size:0.85em;'>plain</span> = appears in all three",
                    ...
                    with col1:
                        render_results(recs_v1, 'Baseline (tags · labels · ratings)', shared_ids)
                    with col2:
                        render_results(recs_v2, 'v2 (+ country · track stats)', shared_ids)
                    with col3:
                        render_results(recs_v3, 'v3 (+ contributors · instruments)', shared_ids)

                    n_only_v1 = len(s1 - s2 - s3)
                    n_only_v2 = len(s2 - s1 - s3)
                    n_only_v3 = len(s3 - s1 - s2)
                    st.caption(
                        f'{len(shared_ids)} in all three · '
                        f'{n_only_v1} only in baseline · '
                        f'{n_only_v2} only in v2 · '
                        f'{n_only_v3} only in v3'
                    )
