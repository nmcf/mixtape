...
All matrices have 2,241,402 rows (the full album universe); albums with no data are zero rows.
...
| `album_tags_matrix.npz` | 2.24M × 3,041 | Direct MusicBrainz album tags, vote-weighted | v1, v2 |
| `album_genre_matrix.npz` | 2.24M × 11,247 | Combined album + artist + label tags (see below) | v3 |
| `album_labels_matrix.npz` | 2.24M × 3,560 | Record label identity, equal weight per label | v1, v2 |
| `album_types_matrix.npz` | 2.24M × 10 | Label type (e.g. original production, imprint) — **not** release type | v1, v2 |
| `album_record_label_matrix.npz` | 2.24M × 3,570 | Labels + types combined (hstack of the two above) | v3 |
| `album_ratings_matrix.npz` | 2.24M × 1 | Bayesian-weighted album rating score | all |
| `album_country_matrix.npz` | 2.24M × 2,263 | One-hot primary artist country | v2, v3 |
| `album_track_stats_matrix.npz` | 2.24M × 12 | Track-length statistics, min-max scaled | v2, v3 |
...
92.2% of albums have fewer than 5 direct tags (measured over the full universe), so artist tags are blended for the large majority. Tags appearing fewer than 10 times across all three sources combined are dropped. Rows are L1-normalised after combining so each album's profile sums to 1.0.
...
Result: 11,247 tag columns vs 3,041 in the original album-only matrix (7.5M non-zero entries vs 3.0M), covering 1,661,288 albums.
...
|-------|-----------------------|----------------------|----------------|
| v1 | 6,612 | 6,612 | 1,352,482 |
| v2 | 8,887 | 8,887 | 2,240,303 |
| v3 | 17,093 | 17,090 | 2,240,363 |
