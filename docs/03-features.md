...
Built by `13-feature-era.ipynb`. Assigns each album a decade/era bin derived from a three-source fallback chain:
...
Coverage: ~97.9% of albums have an era bin. The irreducible 2.1% have no date anywhere in MusicBrainz. The full data review is in `1-EDA/07-EDA-year.ipynb`.
